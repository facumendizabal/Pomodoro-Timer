...
const changePlayIcon = () => {
  //changing the play icon
  const icon = document.querySelector(".buttons__play span");
  let pauseUrl = 'url("./assets/icons/pause.svg")';
  let playUrl = 'url("./assets/icons/play.svg")';

  if (
    session.timer.getStatus() === "stopped" ||
    session.timer.getStatus() === "paused"
  ) {
    icon.style.backgroundImage = playUrl;
  } else if (session.timer.getStatus() === "started") {
    icon.style.backgroundImage = pauseUrl;
  }
};
...
//buttons events
const restartButon = document
  .querySelector(".buttons__rest-time")
...
const playButton = document
  .querySelector(".buttons__play")
  .addEventListener("click", () => {
    session.tooglePlay();
    changePlayIcon();
  });

const skipSessionButton = document
  .querySelector(".buttons__skip-session")
  .addEventListener("click", () => {
    session.timer.end();
    changePlayIcon();
  });
...
// Time setters
const setWorkTimeButton = document
  .querySelector(".accept-work-time button")
  .addEventListener("click", () => {
    workSessionTime =
      document.querySelector(".accept-work-time input").value * 60;
    session.setDisplayTime(session.initialSecToTime());
    restartSession();
  });

const setLongTimeButton = document
  .querySelector(".accept-long-time button")
  .addEventListener("click", () => {
    longBreakTime =
      document.querySelector(".accept-long-time input").value * 60;
    session.setDisplayTime(session.initialSecToTime());
    restartSession();
  });

const setShortTimeButton = document
  .querySelector(".accept-short-time button")
  .addEventListener("click", () => {
    shortBreakTime =
      document.querySelector(".accept-short-time input").value * 60;
    session.setDisplayTime(session.initialSecToTime());
    restartSession();
  });

// implementing logic for the 3 types of times at the end of a session.
// if the session counter is an even number then its time for a short break.
session.timer.on("end", () => {
  ...
  if (session.sessionsCounter === 15) {
    ...
    sessionsDots[session.workSessionsCounter].style.backgroundImage =
      "url('./assets/icons/full-circle.svg')";
